...
{.push dynlib: SdlLib.}
proc sdl_create_window*(title: cstring, w, h: cint; flags: WindowFlag): pointer             {.importc: "SDL_CreateWindow"     .}
proc sdl_create_popup_window*(parent: Window; x, y, w, h: cint; flags: WindowFlag): pointer {.importc: "SDL_CreatePopupWindow".}
proc sdl_destroy_window*(win)                                                               {.importc: "SDL_DestroyWindow"    .}
proc sdl_quit*()                                                                            {.importc: "SDL_Quit"             .}

proc sdl_set_window_position*(win; x, y: cint): SdlBool  {.importc: "SDL_SetWindowPosition".}
proc sdl_set_window_title*(win; title: cstring): SdlBool {.importc: "SDL_SetWindowTitle"   .}
...
proc sdl_get_display_bounds*(d_id; rect: ptr Rect): SdlBool                                            {.importc: "SDL_GetDisplayBounds"               .}
proc sdl_get_display_usable_bounds*(d_id; rect: ptr Rect): SdlBool                                     {.importc: "SDL_GetDisplayUsableBounds"         .}
...
proc sdl_get_closest_fullscreen_display_mode*(d_id; w, h: cint; refresh: cfloat; hd: SdlBool): pointer {.importc: "SDL_GetClosestFullscreenDisplayMode".}
...
proc sdl_set_window_fullscreen_mode*(win; mode: ptr DisplayMode): SdlBool                              {.importc: "SDL_SetWindowFullscreenMode"        .}
...
proc create_window*(title: string; w, h: int32; flags = winNone): (Window, bool) =
    let win = sdl_create_window(cstring title, w, h, flags)
    result[0] = Window win
    result[1] = win == nil

proc create_popup*(parent: Window; x, y, w, h: int32; flags = winNone): (Window, bool) =
    let win = sdl_create_popup_window(parent, x, y, w, h, flags)
    result[0] = Window win
    result[1] = win == nil
