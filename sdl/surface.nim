...
proc SDL_DestroySurface*(surf: Surface) {.importc, cdecl, dynlib: SdlLib.}
...
proc `$`*(surf: Surface): string =
    $(cast[ptr SurfaceObj](surf)[])
...
proc palette*(surf: Surface): Palette =
    ...
    sdl_assert result, &"Failed to get palette for surface ({surf})"
...
proc scale*(surf: Surface; w, h: distinct SomeInteger; mode: ScaleMode = Linear): Surface =
    ...
    sdl_assert result, &"Failed to scale surface to {w}x{h} with mode '{mode}' ({surf})"
